...
ContactService.createAgenda = async (slug) => {
    try {
        ...
        const updatedAgenda = await ContactService.getAgenda('perepau');
...
ContactService.createContact = async (Contact) => {
    try {
       await fetch(`https://playground.4geeks.com/contact/agendas/perepau/contacts`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        return ContactService.getAgenda('perepau')
    } catch (error) {
        console.log(error)
    }
}
